      ...
      {vm.isToday_ && (
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, borderTop: '1px solid var(--rule)', marginBottom: 8 }}>
            {vm.branchRows.map((r) => (
              <div key={r.idx}>
                <button onClick={r.select} style={{ display: 'block', width: '100%', padding: '18px 22px 14px', border: 'none', borderBottom: '1px solid var(--rule)', background: r.rowBg, cursor: 'pointer', textAlign: 'left', color: 'inherit', fontFamily: 'inherit' }}>
                ...
                {r.isSelected && (
                  <div style={{ background: 'var(--bg-sink)', borderBottom: '1px solid var(--rule)', padding: '24px 28px 28px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
                        <div style={{ fontFamily: 'var(--font-family-body)', fontSize: 15, color: 'var(--ink)', fontWeight: 500, letterSpacing: '.04em' }}>{r.branch}</div>
                        <div style={{ fontFamily: 'var(--font-family-body)', fontSize: 11, color: 'var(--ink-3)', letterSpacing: '.04em' }}>{r.startLabel} — {r.endLabel} · {r.durLabel} · {r.tools} tools</div>
                      </div>
                      <button onClick={r.deselect} style={{ background: 'transparent', border: 'none', color: 'var(--ink-3)', fontFamily: 'var(--font-family-body)', fontSize: 18, cursor: 'pointer', lineHeight: 1, padding: '4px 8px' }}>×</button>
                    </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 14, fontFamily: 'var(--font-family-body)', fontSize: 10, color: 'var(--ink-3)', letterSpacing: '.08em' }}>
            ...
            <span style={{ letterSpacing: '.06em', color: 'var(--ink-3)' }}>クリックで詳細を展開</span>
          </div>
        </>
      )}
    </>
